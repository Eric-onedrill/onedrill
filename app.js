...
const ADMIN_EMAILS=[]; // Role resolved via app_roles table only
const BATCH_SIZE=200;
const AUTO_REFRESH_MS=300000;
const GEOCODE_INTERVAL_MS=1100; // >1s for Nominatim rate limit
const TABLE_PAGE_SIZE=100; // Lazy pagination for ticket table

/* ═══════════ 2. UTILITIES ═══════════ */
function debounce(fn,ms){let t;return(...a)=>{clearTimeout(t);t=setTimeout(()=>fn(...a),ms);};}

/** Verifica se algum modal está aberto (robusto: checa display e classe) */
function isAnyModalOpen(){
  return Array.from(document.querySelectorAll('.overlay')).some(ov=>{
    const style=window.getComputedStyle(ov);
    return style.display!=='none'&&style.visibility!=='hidden'&&!ov.classList.contains('hidden');
  });
}
...
let sortCol='ticket',sortAsc=true,tblPage=1;
...
const debouncedTable=debounce(()=>{tblPage=1;renderTable();},250);
...
function clearMapLayersGeneric(mapInst,mkrArr,lineArr,lblArr,clGrp){
  mkrArr.forEach(m=>{m.off();mapInst.removeLayer(m);});
  lineArr.forEach(l=>{l.off();mapInst.removeLayer(l);});
  lblArr.forEach(l=>{mapInst.removeLayer(l);});
  if(clGrp){mapInst.removeLayer(clGrp);}
  mkrArr.length=0;lineArr.length=0;lblArr.length=0;
}

function clearMapLayers(){
  clearMapLayersGeneric(map,mkrs,lines,labels,clusterGroup);
  clusterGroup=null;
}
...
function buildPopup(t,c){
  ...
  const isExp=t.expire&&t.expire!=='—'&&t.status!=='Closed'&&t.status!=='Cancel'&&new Date(t.expire)<new Date();
...
/**
 * Renderiza tickets no mapa (genérico — usado por renderMap e renderSharedMap).
 * @param {Object} opts
 * @param {L.Map} opts.mapInst - Instância do mapa Leaflet
 * @param {Array} opts.filtered - Tickets filtrados para exibir
 * @param {Array} opts.mkrArr - Array de markers (mutado in-place)
 * @param {Array} opts.lineArr - Array de polylines (mutado in-place)
 * @param {Array} opts.lblArr - Array de labels (mutado in-place)
 * @param {boolean} opts.useClusters - Se true, usa MarkerClusterGroup
 * @param {boolean} opts.interactive - Se true, adiciona click handlers (hiT, showPanel)
 * @param {Function} opts.fallbackCenter - Função (projectId) => [lat,lon] pra jitter fallback
 */
function renderMapGeneric(opts){
  const{mapInst,filtered,mkrArr,lineArr,lblArr,useClusters=false,interactive=true,fallbackCenter=null}=opts;
  let clGrp=null;
  if(useClusters){
    clGrp=L.markerClusterGroup({maxClusterRadius:40,spiderfyOnMaxZoom:true,showCoverageOnHover:false,disableClusteringAtZoom:17});
  }
  ...
  for(const t of filtered){
    ...
    if(coords){
      ...
      const m1=L.marker(coords[0],{icon:mi(1)}).addTo(mapInst);
      const m2=L.marker(coords[coords.length-1],{icon:mi(.7)}).addTo(mapInst);
      m1.bindPopup(buildPopup(t,c));m2.bindPopup(buildPopup(t,c));
      if(interactive){m1.on('click',()=>hiT(t.id));m2.on('click',()=>hiT(t.id));}
      mkrArr.push(m1,m2);
      const ln=L.polyline(coords,{color:c,weight:lw,opacity:.92,dashArray:dash}).addTo(mapInst);
      if(interactive)ln.on('click',()=>{hiT(t.id);showPanel(t)});
      lineArr.push(ln);
      const mid=coords[Math.floor(coords.length/2)]||coords[0];
      const lbl=L.marker(mid,{icon:L.divIcon({className:'',html:`<a class="ticket-label" onclick="openTicketDetail(${t.id});return false;" href="#" style="border-left:3px solid ${c}">${esc(t.ticket)}</a>`,iconAnchor:[32,10]})}).addTo(mapInst);
      lblArr.push(lbl);
    }else{
      let pos=t._geocoded||null;
      if(!pos){
        const fc=fallbackCenter?fallbackCenter(t):null;
        const pc=fc||projCenter(t.projectId);
      ...
      const mk=L.marker(pos,{icon:mi});mk.bindPopup(buildPopup(t,c));
      if(interactive)mk.on('click',()=>hiT(t.id));
      mkrArr.push(mk);
      if(clGrp)clGrp.addLayer(mk);
      else mk.addTo(mapInst);
      const lbl=L.marker(pos,{icon:L.divIcon({className:'',html:`<a class="ticket-label" onclick="openTicketDetail(${t.id});return false;" href="#" style="margin-top:12px;display:block;border-left:3px solid ${c}">${esc(t.ticket)}</a>`,iconAnchor:[32,-2]})}).addTo(mapInst);
      lblArr.push(lbl);
    }
  }
  if(clGrp)mapInst.addLayer(clGrp);
  return clGrp;
}
...
async function renderMap(){
  if(!map)return;
  clearMapLayers();
  clusterGroup=renderMapGeneric({
    mapInst:map,filtered:mapFiltered(),mkrArr:mkrs,lineArr:lines,lblArr:labels,
    useClusters:true,interactive:true
  });
}
...
function showPanel(t){
  ...
  currentPanelId=t.id;
  const isExp=t.expire&&t.expire!=='—'&&t.status!=='Closed'&&t.status!=='Cancel'&&new Date(t.expire)<new Date();
  document.getElementById('ptitle-txt').textContent=t.ticket;
...
function renderList(){
  ...
  document.getElementById('tlist').innerHTML=f.length?f.map(t=>
    ...
    +`<div class="tcard-top"><span class="tcard-num">${esc(t.ticket)}</span><span class="sbadge b-${t.status.toLowerCase()}">${esc(t.status)}</span></div>`
...
function renderTable(){
  ...
  const pageEnd=tblPage*TABLE_PAGE_SIZE;
  const visible=f.slice(0,pageEnd);
  const hasMore=pageEnd<totalCount;

  document.getElementById('tbl-count').textContent=`${totalCount} tickets · ${totalFt.toLocaleString()} ft`;
  document.getElementById('tbl-body').innerHTML=visible.map(t=>{
    const pends=getTicketPendingUtils(String(t.ticket).trim());
    ...
    const pendChips=pendNames.length
      ?`<div style="display:flex;flex-wrap:wrap;gap:2px;margin-top:3px">`
        ...
        +`</div>`
      :'';
    const rs=riskScore(t);const rl=riskLabel(rs);
    return`<tr onclick="openTicketDetail(${t.id})">`
      +`<td style="font-family:var(--mono);font-weight:500">${esc(t.ticket)}</td>`
      ...
      +`<td><span style="font-size:10px;padding:2px 7px;border-radius:10px;background:${rl.bg};color:${rl.color};border:1px solid ${rl.border};font-weight:600;font-family:var(--mono)">${rs} ${rl.label}</span></td>`
  ...
  +(hasMore?`<tr><td colspan="10" style="text-align:center;padding:12px"><button class="btn btn-sm" onclick="tblPage++;renderTable()" style="min-width:200px">Carregar mais (${totalCount-pageEnd} restantes)</button></td></tr>`:'');
}
function sortBy(col){sortAsc=sortCol===col?!sortAsc:true;sortCol=col;tblPage=1;renderTable();}
...
function enterSharedView(pid){
  ...
  const expiring3d=ts.filter(t=>{if(!t.expire||t.expire==='—'||t.status==='Closed'||t.status==='Cancel')return false;const d=new Date(t.expire);const diff=(d-now)/86400000;return diff>=0&&diff<=3;});
  const expiring7d=ts.filter(t=>{if(!t.expire||t.expire==='—'||t.status==='Closed'||t.status==='Cancel')return false;const d=new Date(t.expire);const diff=(d-now)/86400000;return diff>3&&diff<=7;});
...
function clearSharedMapLayers(){
  clearMapLayersGeneric(shMap,shMkrs,shLines,shLabels,null);
}
...
function renderSharedMap(){
  ...
  renderMapGeneric({
    mapInst:shMap,filtered:sharedFiltered(),mkrArr:shMkrs,lineArr:shLines,lblArr:shLabels,
    useClusters:false,interactive:false,
    fallbackCenter:t=>{
      const p=projects.find(x=>x.id===sharedProjectId);
      return p?.centerCoords||null;
    }
  });
}
