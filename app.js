...
function renderTable(){
  ...
  document.getElementById('tbl-body').innerHTML=f.map(t=>{
    ...
    const pendChips=pendNames.length
      ...
      :'';
    const es=effectiveStatus(t);const inGrace=isRenewed(t)&&isInRenewalGrace(t);
    // EXPIRA = "clear até quando": se está Clear SÓ pela carência (o novo ainda não clareou),
    // vale até o CUTOVER do antigo; se o NOVO já clareou de verdade (ou não é carência),
    // vale até o expire do próprio ticket.
    ...
    return`<tr onclick="openTicketDetail(${t.id})">`
      ...
      +`<td style="font-family:var(--mono);font-size:12px">${esc(_vencCol)}${_viaCarencia?' <span style="font-size:9px;color:#7c3aed">carência</span>':''}</td>`
